...
def train_strikeout_model(df, features, target='strikeouts', train_years=(2019, 2020, 2021, 2022), 
...
def train_outs_model(df, features, target='outs', train_years=(2019, 2020, 2021, 2022), 
                     test_years=(2023, 2024), random_state=42):
    """
    Train a model to predict outs using time-based splitting
    ...
    Returns:
        dict: Dictionary with model, scaler, metrics, and feature importance
    """
    logger.info(f"Training outs prediction model with {len(features)} features")
    logger.info(f"Training years: {train_years}, Testing years: {test_years}")
    ...
    # Train Random Forest model - works well for count data like outs
    rf = RandomForestRegressor(
        n_estimators=100, 
        max_depth=None,
        min_samples_split=2,
    ...
    # For count data like outs, round predictions to nearest integer
    ...
    logger.info(f"Model metrics: RMSE={metrics['rmse']:.3f}, MAE={metrics['mae']:.3f}, R²={metrics['r2']:.3f}")
    logger.info(f"Rounded metrics: RMSE={metrics['rmse_rounded']:.3f}, MAE={metrics['mae_rounded']:.3f}")
    logger.info(f"Top features by importance: {', '.join(importance['feature'].head(3).tolist())}")
