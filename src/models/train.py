...
def tune_lightgbm_hyperparameters(X_train, y_train, cv=3, random_state=StrikeoutModelConfig.RANDOM_STATE):
    """
    Perform basic hyperparameter tuning for LightGBM
    ...
    """
    logger.info("Tuning hyperparameters for LightGBM model...")
...
def train_strikeout_model(df, features, target='strikeouts', 
    ...
    """
    Train a LightGBM model to predict strikeouts using time-based splitting
    ...
    """
    logger.info(f"Training LightGBM strikeout prediction model with {len(features)} features")
    ...
    if tune_hyperparameters:
        params = tune_lightgbm_hyperparameters(X_train, y_train, random_state=random_state)
    else:
    ...
    # Add random state to parameters
