...
def get_starting_pitchers(conn) -> pd.DataFrame:
    """Return starting pitcher IDs per game and team."""
    query = """
    WITH pitch_team AS (
        SELECT
            game_pk,
    ...
    SELECT game_pk, pitching_team, opponent_team, pitcher
    FROM ranked
    WHERE rn = 1
    """
...
def load_batter_game(conn, game_pk: int, pitcher: int) -> pd.DataFrame:
    q = (
        "SELECT batter, at_bat_number, pitch_number, events, description, "
        "balls, strikes, woba_value, woba_denom FROM statcast_batters "
        "WHERE game_pk = ? AND pitcher = ?"
    )
