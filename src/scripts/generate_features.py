...
try:
    ...
    from src.features.pitcher_features import (
        calculate_pitcher_rolling_features,
        calculate_pitcher_rest_days,
    )
    ...
    logger = setup_logger(
        "generate_features", LogConfig.LOG_DIR / "generate_features.log"
    )
...
OPPONENT_METRICS_FOR_ROLLING = [
    ...
    "k_percent_bat",
    "swinging_strike_percent",  # Assuming SwStr% is opponent pitching stat
    "woba_bat",
    "iso_bat",
    "babip_bat",
    "hard_hit_percent",
...
def load_data_from_db(query: str, db_path: Path, optimize: bool = True) -> pd.DataFrame:
    ...
    try:
        ...
        logger.error(
            f"Failed to load data with query: {query[:100]}... Error: {e}",
            exc_info=True,
        )
    return df


def calculate_multi_window_rolling(
    df, group_col, date_col, metrics, windows, min_periods, shift_periods=1
):
    """
    ...
    logger.info(
        f"Calculating multi-window {windows} rolling for group '{group_col}' on {len(metrics)} metrics..."
    )
    ...
    if not pd.api.types.is_datetime64_any_dtype(df_internal[date_col]):
        try:
            df_internal[date_col] = pd.to_datetime(df_internal[date_col])
        except Exception as e:
    ...
    for metric in metrics:
        if metric not in df_sorted.columns:
            logger.warning(
                f"Metric '{metric}' not found in dataframe for rolling calculation. Skipping."
            )
            continue
        ...
        if metric_series.isnull().all():
            logger.warning(
                f"Metric '{metric}' is entirely non-numeric or NaN after coercion. Skipping rolling."
            )
            continue
        ...
        for window in windows:
            ...
            current_min_periods = max(
                1, min(min_periods, window)
            )  # Allow min_periods=window
            try:
                ...
                logger.error(
                    f"Error calculating rolling for {metric} window {window}: {e}",
                    exc_info=True,
                )
...
def generate_features(
    ...
    if prediction_date_str:
        try:
            ...
            max_hist_date_str = (prediction_date - timedelta(days=1)).strftime(
                "%Y-%m-%d"
            )
            logger.info(
                f"Prediction Date: {prediction_date_str}. Loading history up to {max_hist_date_str}."
            )
        except ValueError:
            logger.error(
                f"Invalid prediction date format: {prediction_date_str}. Use YYYY-MM-DD."
            )
            return
    else:
    ...
    try:
        ...
        logger.error(
            f"Aggregation function not found (check imports?): {ne}", exc_info=True
        )
        return
    except Exception as agg_e:
        logger.error(f"Error during aggregation: {agg_e}", exc_info=True)
        return
    gc.collect()
    ...
    try:
        with DBConnection() as conn:
            db_connection_successful = True  # Test connection
    except Exception as db_err:
        logger.error(f"Failed initial DB connection test: {db_err}")
        return
    ...
    team_to_ballpark_map = (
        team_map_df.set_index("team_abbr")["ballpark"].to_dict()
        if not team_map_df.empty
        else {}
    )
    if not team_to_ballpark_map:
        logger.warning("Team mapping empty.")
    ...
    try:
        with DBConnection() as conn:
            starter_cols_avail = pd.read_sql_query(
                f"SELECT * FROM {pitcher_base_table} LIMIT 1", conn
            ).columns.tolist()
    except Exception as e:
        logger.error(f"Cannot read columns from {pitcher_base_table}: {e}")
        return

    # Define columns needed from starter table
    pitcher_metrics_needed = (
        set(PITCHER_METRICS_FOR_ROLLING)
        | set(BALLPARK_METRICS_FOR_ROLLING)
        | set(UMPIRE_METRICS_FOR_ROLLING)
        | set(LEAGUE_AVERAGE_METRICS)
    )
    pitcher_base_cols = [
        "pitcher_id",
        "game_date",
    ...
    pitcher_cols_to_load = list(
        set(pitcher_base_cols) | pitcher_metrics_needed | set(target_cols)
    )
    pitcher_cols_to_load_str = ", ".join(
        [f'"{c}"' for c in pitcher_cols_to_load if c in starter_cols_avail]
    )
    if "pitcher_id" not in pitcher_cols_to_load_str:
        logger.error(
            f"'pitcher_id' missing from available columns in {pitcher_base_table}"
        )
        return
    if not pitcher_cols_to_load_str:
        logger.error(
            f"No pitcher columns could be identified for loading from {pitcher_base_table}."
        )
        return
    ...
    try:
        with DBConnection() as conn:
            team_cols_avail = pd.read_sql_query(
                f"SELECT * FROM {team_stats_table} LIMIT 1", conn
            ).columns.tolist()
    except Exception as e:
        logger.warning(f"Cannot read columns from {team_stats_table}: {e}")
        team_cols_avail = []

    # Update opponent metrics based on what's likely in game_level_team_stats
    opponent_metrics_likely = [
        "k_percent_bat",
        "swinging_strike_percent",
        "woba_bat",
        "iso_bat",
    ...
    opponent_metrics_to_use = [
        m for m in opponent_metrics_likely if m in team_cols_avail
    ]
    if not opponent_metrics_to_use:
        logger.warning(f"No opponent batting metrics found in {team_stats_table}")

    team_base_cols = ["team", "game_date", "game_pk", "home_team"]
    team_cols_to_load = list(set(team_base_cols) | set(opponent_metrics_to_use))
    team_cols_to_load_str = ", ".join(
        [f'"{c}"' for c in team_cols_to_load if c in team_cols_avail]
    )
    team_hist_df = (
        load_data_from_db(
            f"SELECT {team_cols_to_load_str} FROM {team_stats_table} WHERE DATE(game_date) <= '{max_hist_date_str}'",
            db_path,
        )
        if team_cols_to_load_str
        else pd.DataFrame()
    )
    ...
    if umpire_hist_df.empty:
        logger.warning(f"Historical umpire data table '{umpire_table}' is empty.")
    # <<< END UMPIRE HISTORY LOAD ---

    # --- Load Catcher Framing Data and Game Catchers ---
    catcher_framing_df = load_data_from_db("SELECT * FROM catcher_framing", db_path, optimize=False)
    game_catchers_df = load_data_from_db("SELECT * FROM game_catchers", db_path, optimize=False)

    # --- Data Validation and Prep ---
    if pitcher_hist_df.empty:
        logger.error(
            f"Starter pitcher history empty (loaded from {pitcher_base_table}). Cannot proceed."
        )
        return
    ...
    pitcher_hist_df["season"] = pitcher_hist_df["game_date"].dt.year
    ...
    if "team" in pitcher_hist_df.columns and "home_team" in pitcher_hist_df.columns:
        pitcher_hist_df["is_home"] = (
            pitcher_hist_df["team"] == pitcher_hist_df["home_team"]
        ).astype(int)
        logger.info("Derived 'is_home' column for starters.")
    else:
        logger.error(
            "'team' or 'home_team' column missing from starter history. Cannot derive 'is_home'."
        )
    ...
    if "home_team" in pitcher_hist_df.columns:
        pitcher_hist_df["ballpark"] = (
            pitcher_hist_df["home_team"]
            .map(team_to_ballpark_map)
            .fillna("Unknown Park")
        )
        logger.info("Derived 'ballpark' column for starters.")
    else:
        logger.warning(
            "Missing 'home_team' in starter history, cannot map ballparks accurately."
        )
        pitcher_hist_df["ballpark"] = "Unknown Park"

    # Merge catcher IDs from game_catchers table
    if not game_catchers_df.empty:
        pitcher_hist_df = pd.merge(
            pitcher_hist_df,
            game_catchers_df,
            on=["game_pk", "pitcher_id"],
            how="left",
        )
        logger.info("Merged catcher_id onto pitcher history.")
    else:
        pitcher_hist_df["catcher_id"] = np.nan

    # Merge season-level framing runs
    if not catcher_framing_df.empty:
        pitcher_hist_df = pd.merge(
            pitcher_hist_df,
            catcher_framing_df,
        ...
        pitcher_hist_df["framing_runs"] = np.nan
    ...
    missing_req_cols = [
        c for c in required_for_features if c not in pitcher_hist_df.columns
    ]
    if missing_req_cols:
        logger.error(
            f"Starter history DF is missing essential columns: {missing_req_cols}. Aborting feature calculation."
        )
        return
    ...
    if not pitcher_hist_df.empty and LEAGUE_AVERAGE_METRICS:
        metrics_for_league_avg = [
            m for m in LEAGUE_AVERAGE_METRICS if m in pitcher_hist_df.columns
        ]
        if not metrics_for_league_avg:
            logger.warning(
                "None of the specified LEAGUE_AVERAGE_METRICS found in starter data."
            )
        else:
            # Ensure metrics are numeric
            for metric in metrics_for_league_avg:
                pitcher_hist_df.loc[:, metric] = pd.to_numeric(
                    pitcher_hist_df[metric], errors="coerce"
                )

            daily_league_stats = (
                pitcher_hist_df.groupby("game_date")[metrics_for_league_avg]
                .agg(["mean", "std"])
                .sort_index()
                .shift(1)  # Use previous day's league stats to avoid leakage
                .reset_index()
            )
            daily_league_stats.columns = [
                "_".join(col).strip() + "_league_daily" if col[1] else col[0]
                for col in daily_league_stats.columns.values
            ]

            original_index = pitcher_hist_df.index
            pitcher_hist_df = pd.merge(
                pitcher_hist_df.reset_index(),
                daily_league_stats,
                on="game_date",
                how="left",
            ).set_index("index")
            ...
            league_avg_cols = {
                col: col for col in daily_league_stats.columns if col != "game_date"
            }
            logger.info(
                f"Added daily league average columns (starter-based): {list(league_avg_cols.keys())}"
            )
            ...
            for col in league_avg_cols:
                if pitcher_hist_df[col].isnull().any():
                    ...
                    logger.warning(
                        f"Imputing {pitcher_hist_df[col].isnull().sum()} NaNs in league avg column '{col}' with {fill_value:.4f}"
                    )
                    pitcher_hist_df.loc[:, col] = pitcher_hist_df[col].fillna(
                        fill_value
                    )
    else:
        logger.warning("Starter history empty or no league average metrics defined.")
    # --- End League Average Calculation ---

    logger.info(
        f"Data loading & prep finished in {(datetime.now() - start_load_time).total_seconds():.2f}s."
    )
    gc.collect()

    # --- STEP 2 & 3: Calculate Historical Rolling Features ---
    logger.info(
        f"STEP 2&3: Calculating historical rolling features (Windows: {ROLLING_WINDOWS}) (Base: Starters)..."
    )
    ...
    pitcher_metrics_avail = [
        m for m in PITCHER_METRICS_FOR_ROLLING if m in pitcher_hist_df.columns
    ]
    if not pitcher_metrics_avail:
        logger.warning(
            "No pitcher metrics found in starter data for rolling calculations."
        )
    ...
    all_rename_maps["pitcher"] = {
        col: col for col in pitcher_rolling_df.columns
    ...
    if not team_hist_df.empty:
        ...
        logger.warning(
            "Skipping opponent rolling features calculation as team history is empty."
        )

    # Ballpark Rolling Features (Now based on Starters only)
    ballpark_metrics_avail = [
        m for m in BALLPARK_METRICS_FOR_ROLLING if m in pitcher_hist_df.columns
    ]
    if not ballpark_metrics_avail:
        logger.warning(
            "No ballpark metrics found in starter data for rolling calculations."
        )
    ...
    umpire_metrics_avail = [
        m for m in UMPIRE_METRICS_FOR_ROLLING if m in pitcher_hist_df.columns
    ]
    if not umpire_metrics_avail:
        logger.warning(
            "No umpire metrics found in starter data for rolling calculations."
        )
    ...
    if not catcher_metrics_avail:
        logger.warning("No catcher metrics found for rolling calculations.")
    catcher_rolling_df, catcher_rename_map = calculate_catcher_rolling_features(
        catcher_hist_df=pitcher_hist_df,
        group_col="catcher_id",
        date_col="game_date",
        metrics=catcher_metrics_avail,
        windows=ROLLING_WINDOWS,
        min_periods=MIN_ROLLING_PERIODS,
        calculate_multi_window_rolling=calculate_multi_window_rolling,
    )
    ...
    if mode == "HISTORICAL":
        ...
        if "p_days_rest" in pitcher_hist_df.columns:
            base_cols.append("p_days_rest")
        base_cols.extend(league_avg_cols.keys())  # Add league avg cols

        # Add target vars if they exist
        if "strikeouts" in pitcher_hist_df.columns:
            base_cols.append("strikeouts")
        if "batters_faced" in pitcher_hist_df.columns:
            base_cols.append("batters_faced")
        ...
        if missing_base_cols:
            logger.warning(
                f"Missing base columns from pitcher_hist_df: {missing_base_cols}"
            )
        ...
        if not umpire_hist_df.empty and "home_plate_umpire" in umpire_hist_df.columns:
            umpire_lookup = umpire_hist_df[
                ["game_date", "home_team", "away_team", "home_plate_umpire"]
            ].drop_duplicates()
            if not pd.api.types.is_datetime64_any_dtype(umpire_lookup["game_date"]):
                umpire_lookup["game_date"] = pd.to_datetime(umpire_lookup["game_date"])
            if not pd.api.types.is_datetime64_any_dtype(final_features_df["game_date"]):
                final_features_df["game_date"] = pd.to_datetime(
                    final_features_df["game_date"]
                )
            ...
            if all(key in final_features_df.columns for key in merge_keys_ump):
                final_features_df = pd.merge(
                    final_features_df, umpire_lookup, on=merge_keys_ump, how="left"
                )
                logger.debug(
                    f"Added home_plate_umpire column. {final_features_df['home_plate_umpire'].isnull().sum()} nulls."
                )
            else:
                logger.warning(
                    f"Missing keys {merge_keys_ump} for umpire name merge. Skipping."
                )
        ...
        if (
            "pitcher" in all_rolling_features
            and not all_rolling_features["pitcher"].empty
        ):
            final_features_df = pd.concat(
                [final_features_df, all_rolling_features["pitcher"]], axis=1
            )
            logger.debug("Merged pitcher rolling features (starter-based).")

        # Merge Opponent features (derived from team_stats)
        if (
            "team" in all_rolling_features
            and "opponent" in all_rename_maps
            and not all_rolling_features["team"].empty
        ):
        ...
        if (
            "ballpark" in all_rolling_features
            and "ballpark" in all_rename_maps
            and not all_rolling_features["ballpark"].empty
        ):
        ...
        if (
            ...
            and "catcher" in all_rename_maps
            and not all_rolling_features["catcher"].empty
        ):
            final_features_df = merge_catcher_features_historical(
                final_features_df=final_features_df,
                catcher_rolling_df=all_rolling_features["catcher"],
        ...
        if (
            ...
            and not all_rolling_features["umpire"].empty
        ):
            final_features_df = pd.concat(
                [final_features_df, all_rolling_features["umpire"]], axis=1
            )
            logger.debug("Merged umpire rolling features (starter-based).")
        else:
            logger.warning(
                "No umpire rolling features calculated, skipping historical merge."
            )
            if "umpire" in all_rename_maps:
                for col in all_rename_maps["umpire"].values():
                    final_features_df[col] = np.nan

        # Add season
        if "game_date" in final_features_df.columns:
            final_features_df["season"] = pd.to_datetime(
                final_features_df["game_date"]
            ).dt.year

    elif mode == "PREDICTION":
        logger.info(
            "STEP 4 [PREDICTION]: Merging latest features onto prediction baseline (Base: Starters)..."
        )
        # Load schedule, build baseline for probable starters
        schedule_query = (
            f"SELECT * FROM mlb_api WHERE DATE(game_date) = '{prediction_date_str}'"
        )
        schedule_df = load_data_from_db(schedule_query, db_path, optimize=False)
        if schedule_df.empty:
            logger.error("Prediction schedule missing.")
            return
        baseline_data = []
        for _, game in schedule_df.iterrows():
            game_date_dt = pd.to_datetime(game["game_date"])
            # Ensure IDs are numeric, handle potential errors
            home_pid = pd.to_numeric(
                game.get("home_probable_pitcher_id"), errors="coerce"
            )
            away_pid = pd.to_numeric(
                game.get("away_probable_pitcher_id"), errors="coerce"
            )
        ...
        latest_ballpark_indices = (
            pitcher_hist_df.sort_values("game_date")
            .drop_duplicates(subset=["ballpark"], keep="last")
            .index
        )
        latest_catcher_indices = (
            pitcher_hist_df.sort_values("game_date")
            .drop_duplicates(subset=["catcher_id"], keep="last")
            .index
        )
        ...
        temp_ump_df = (
            pd.DataFrame()
        )  # To hold umpire names aligned with pitcher_hist_df index
        if not umpire_hist_df.empty and "home_plate_umpire" in umpire_hist_df.columns:
            temp_ump_lookup = umpire_hist_df[
                ["game_date", "home_team", "away_team", "home_plate_umpire"]
            ].drop_duplicates()
            if not pd.api.types.is_datetime64_any_dtype(temp_ump_lookup["game_date"]):
                temp_ump_lookup["game_date"] = pd.to_datetime(
                    temp_ump_lookup["game_date"]
                )
            ...
            if all(key in pitcher_hist_df.columns for key in merge_keys_ump_pred):
                temp_ump_df = pd.merge(
                    pitcher_hist_df[merge_keys_ump_pred].reset_index(),
                    temp_ump_lookup,
                    on=merge_keys_ump_pred,
                    how="left",
                ).set_index("index")
                temp_ump_df.index.name = None
            else:
                logger.warning(
                    f"Missing keys {merge_keys_ump_pred} in starter history for umpire mapping."
                )

            if "home_plate_umpire" in temp_ump_df.columns:
                temp_ump_df_for_sort = pd.concat(
                    [
                        temp_ump_df[["home_plate_umpire"]],
                        pitcher_hist_df[["game_date"]],
                    ],
                    axis=1,
                )
                latest_umpire_indices = (
                    temp_ump_df_for_sort.dropna(subset=["home_plate_umpire"])
                    .sort_values("game_date")
                    .drop_duplicates(subset=["home_plate_umpire"], keep="last")
                    .index
                )

        # Get latest rolling features DataFrames using indices
        latest_pitcher_rolling = (
            all_rolling_features.get("pitcher", pd.DataFrame()).loc[
                latest_pitcher_indices
            ]
            if "pitcher" in all_rolling_features
            else pd.DataFrame()
        )
        latest_opponent_rolling = (
            all_rolling_features.get("team", pd.DataFrame()).loc[latest_team_indices]
            if ("team" in all_rolling_features and not latest_team_indices.empty)
            else pd.DataFrame()
        )
        latest_ballpark_rolling = (
            all_rolling_features.get("ballpark", pd.DataFrame()).loc[
                latest_ballpark_indices
            ]
            if "ballpark" in all_rolling_features
            else pd.DataFrame()
        )
        latest_catcher_rolling = (
            all_rolling_features.get("catcher", pd.DataFrame()).loc[latest_catcher_indices]
            if ("catcher" in all_rolling_features and not latest_catcher_indices.empty)
            else pd.DataFrame()
        )
        latest_umpire_rolling = (
            all_rolling_features.get("umpire", pd.DataFrame()).loc[
                latest_umpire_indices
            ]
            if ("umpire" in all_rolling_features and not latest_umpire_indices.empty)
            else pd.DataFrame()
        )
        ...
        if not latest_pitcher_rolling.empty:
            latest_pitcher_rolling["pitcher_id"] = pitcher_hist_df.loc[
                latest_pitcher_rolling.index, "pitcher_id"
            ]
        if not latest_opponent_rolling.empty:
            latest_opponent_rolling["team"] = team_hist_df.loc[
                latest_opponent_rolling.index, "team"
            ]
        if not latest_ballpark_rolling.empty:
            latest_ballpark_rolling["ballpark"] = pitcher_hist_df.loc[
                latest_ballpark_rolling.index, "ballpark"
            ]
        if not latest_catcher_rolling.empty:
            latest_catcher_rolling["catcher_id"] = pitcher_hist_df.loc[
                latest_catcher_rolling.index, "catcher_id"
            ]
        if (
            not latest_umpire_rolling.empty
            and not temp_ump_df.empty
            and "home_plate_umpire" in temp_ump_df.columns
        ):
            latest_umpire_rolling["home_plate_umpire"] = temp_ump_df.loc[
                latest_umpire_rolling.index, "home_plate_umpire"
            ]
        ...
        if league_avg_cols and not daily_league_stats.empty:
            ...
            logger.info(
                f"Using latest available daily league averages (starter-based) from: {latest_league_avg_date.strftime('%Y-%m-%d')}"
            )
            latest_league_avgs = daily_league_stats[
                daily_league_stats["game_date"] == latest_league_avg_date
            ]
            if not latest_league_avgs.empty:
                ...
                for col_name, value in latest_league_avg_dict.items():
                    ...
                    logger.debug(
                        f"Added prediction league avg column '{col_name}' with value {value}"
                    )
            else:
                logger.warning(
                    f"No league average data found for date {latest_league_avg_date}."
                )
        else:
            logger.warning("No league average data available to merge for prediction.")

        # Calculate Days Rest (based on starter history)
        last_game_dates = pitcher_hist_df.sort_values("game_date").drop_duplicates(
            subset=["pitcher_id"], keep="last"
        )[["pitcher_id", "game_date"]]
        final_features_df = pd.merge(
            final_features_df,
            last_game_dates.rename(columns={"game_date": "last_game_date"}),
            on="pitcher_id",
            how="left",
        )
        if not game_catchers_df.empty:
            last_catchers = pitcher_hist_df.sort_values("game_date").drop_duplicates(
                subset=["pitcher_id"], keep="last"
            ...
            final_features_df["catcher_id"] = np.nan
        final_features_df["last_game_date"] = pd.to_datetime(
            final_features_df["last_game_date"]
        )
        if not pd.api.types.is_datetime64_any_dtype(final_features_df["game_date"]):
            final_features_df["game_date"] = pd.to_datetime(
                final_features_df["game_date"]
            )
        final_features_df["p_days_rest"] = (
            final_features_df["game_date"] - final_features_df["last_game_date"]
        ).dt.days
        final_features_df = final_features_df.drop(columns=["last_game_date"])

        # Get Pitcher Handedness (based on starter history)
        pitcher_throws = pitcher_hist_df.dropna(subset=["p_throws"]).drop_duplicates(
            subset=["pitcher_id"], keep="last"
        )[["pitcher_id", "p_throws"]]
        final_features_df = pd.merge(
            final_features_df, pitcher_throws, on="pitcher_id", how="left"
        )
        final_features_df["p_throws"] = final_features_df["p_throws"].fillna(
            "R"
        )  # Assume R if unknown
        ...
        if not umpire_pred_lookup.empty:
            final_features_df = pd.merge(
                final_features_df, umpire_pred_lookup, on="game_pk", how="left"
            )
            logger.info(
                f"Looked up umpires for prediction date. Found assignments for {len(final_features_df) - final_features_df['home_plate_umpire'].isnull().sum()} games."
            )
        else:
            logger.warning(
                f"No umpire data found in {umpire_table} for prediction date {prediction_date_str}."
            )
            final_features_df["home_plate_umpire"] = np.nan
        ...
        if "pitcher" in all_rename_maps and not latest_pitcher_rolling.empty:
            ...
            p_cols_to_merge = [
                col for col in p_cols_to_merge if col in latest_pitcher_rolling.columns
            ]
            if len(p_cols_to_merge) > 1:
                final_features_df = pd.merge(
                    final_features_df,
                    latest_pitcher_rolling[p_cols_to_merge],
                    on="pitcher_id",
                    how="left",
                )
        ...
        if "opponent" in all_rename_maps and not latest_opponent_rolling.empty:
            final_features_df = merge_opponent_features_prediction(
                final_features_df=final_features_df,
                latest_opponent_rolling=latest_opponent_rolling,
                opp_rename_map=all_rename_maps["opponent"],
        ...
        if "ballpark" in all_rename_maps and not latest_ballpark_rolling.empty:
            final_features_df = merge_ballpark_features_prediction(
                final_features_df=final_features_df,
                latest_ballpark_rolling=latest_ballpark_rolling,
                bpark_rename_map=all_rename_maps["ballpark"],
            )

        # Catcher Features
        if "catcher" in all_rename_maps and not latest_catcher_rolling.empty:
            final_features_df = merge_catcher_features_prediction(
                final_features_df=final_features_df,
                latest_catcher_rolling=latest_catcher_rolling,
                catcher_rename_map=all_rename_maps["catcher"],
            )
        ...
        if "umpire" in all_rename_maps and not latest_umpire_rolling.empty:
            ...
            ump_cols_to_merge = [
                col for col in ump_cols_to_merge if col in latest_umpire_rolling.columns
            ]
            if (
                len(ump_cols_to_merge) > 1
                and "home_plate_umpire" in final_features_df.columns
            ):
                final_features_df = pd.merge(
                    final_features_df,
                    latest_umpire_rolling[ump_cols_to_merge],
                ...
                logger.warning(
                    "Could not merge latest umpire features (missing key or data)."
                )
        else:
    ...
    if final_features_df.empty:
        logger.error("Features empty before final cleanup.")
        return
    ...
    expected_target_cols = (
        ["strikeouts", "batters_faced", "season"] if mode == "HISTORICAL" else []
    )

    # Get expected rolling columns from rename maps
    expected_p_roll_cols = list(
        all_rename_maps.get("pitcher", {}).keys()
    )  # Use keys if no rename map
    ...
    if mode == "PREDICTION":
        base_opp_metrics = [
            m for m in opponent_metrics_likely if "_vs_" not in m
        ]  # Use base metrics
        expected_opp_roll_cols = [
            col for col in expected_opp_roll_cols_base if "_vs_" not in col
        ]  # Base rolling
        # Add vs_pitcher cols if they were created by merge_opponent_features_prediction
        expected_opp_roll_cols += [
            f"opp_roll{w}g_{m}_vs_pitcher"
            for w in ROLLING_WINDOWS
            for m in base_opp_metrics
        ]
        # Remove duplicates just in case
        expected_opp_roll_cols = sorted(list(set(expected_opp_roll_cols)))
    else:
        expected_opp_roll_cols = expected_opp_roll_cols_base

    # Combine all expected columns
    expected_cols = list(
        set(
            expected_base_cols
            + expected_league_avg_cols
            + expected_target_cols
            + expected_p_roll_cols
            + expected_opp_roll_cols
    ...
    for col in priority_cols:
        if col in present_cols:
            ...
            logger.warning(
                f"Adding missing expected base/target/league column '{col}' as NaN."
            )
            final_features_df[col] = np.nan
            final_cols_ordered.append(col)
    # Add rolling
        ...
        for col in col_list:
            if col in expected_cols:
                if col not in present_cols:
                    logger.warning(
                        f"Adding missing expected rolling column '{col}' as NaN."
                    )
                    final_features_df[col] = np.nan
    ...
    final_cols_ordered = [
        col for col in final_cols_ordered if col in final_features_df.columns
    ]
    ...
    try:
        with DBConnection() as conn:
            if mode == "HISTORICAL":
                logger.info(
                    f"Splitting historical data into train ({train_years}) and test ({test_years})..."
                )
                if "season" not in final_features_df.columns:
                    logger.error("'season' column missing, cannot split by year.")
                else:
                    train_df = final_features_df[
                        final_features_df["season"].isin(train_years)
                    ].copy()
                    test_df = final_features_df[
                        final_features_df["season"].isin(test_years)
                    ].copy()
                    logger.info(
                        f"Saving {len(train_df)} training rows to '{output_table_train}'..."
                    )
                    conn.execute(f"DROP TABLE IF EXISTS {output_table_train}")
                    train_df.to_sql(
                        output_table_train, conn, if_exists="replace", index=False
                    )
                    conn.commit()
                    logger.info(
                        f"Saving {len(test_df)} test rows to '{output_table_test}'..."
                    )
                    conn.execute(f"DROP TABLE IF EXISTS {output_table_test}")
                    test_df.to_sql(
                        output_table_test, conn, if_exists="replace", index=False
                    )
                    conn.commit()
            elif mode == "PREDICTION":
                logger.info(
                    f"Saving {len(final_features_df)} prediction rows with {len(final_features_df.columns)} columns to '{output_table_pred}'..."
                )
                conn.execute(f"DROP TABLE IF EXISTS {output_table_pred}")
                final_features_df.to_sql(
                    output_table_pred, conn, if_exists="replace", index=False
                )
                conn.commit()
                logger.debug(
                    f"Final prediction columns: {final_features_df.columns.tolist()}"
                )
    except Exception as e:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate MLB Features (Base: Starters) for Training/Prediction."
    )
    parser.add_argument(
        "--prediction-date",
        type=str,
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Years for training set. Overrides config.",
    )
    parser.add_argument(
        "--test-years",
        type=int,
        nargs="+",
        default=None,
        help="Years for test set. Overrides config.",
    )
    ...
    train_years_to_use = (
        args.train_years
        if args.train_years
        else StrikeoutModelConfig.DEFAULT_TRAIN_YEARS
    )
    test_years_to_use = (
        args.test_years if args.test_years else StrikeoutModelConfig.DEFAULT_TEST_YEARS
    )
