...
1. **Data Fetching**
    ...
    * Run `python -m src.scripts.data_cli --parallel` to download Statcast and MLB API data. Without `--date` it resumes historical backfill from the last checkpoint and fetches up through yesterday.
    * To fetch a single day only, supply `--date YYYY-MM-DD` (and optionally `--parallel`).
