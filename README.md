...
2. **Aggregation & Feature Engineering**
    ...
    * Execute `python -m src.scripts.run_feature_engineering` to build all aggregated tables
      by running `engineer_pitcher_features`, `engineer_opponent_features`,
      `engineer_contextual_features`, `engineer_lineup_trends` and finally
...
sequentially calls `engineer_pitcher_features`, `engineer_opponent_features`,
`engineer_contextual_features`, `engineer_lineup_trends` and then
`build_model_features`. The `lineup_trends` table must be present for the final
join:
